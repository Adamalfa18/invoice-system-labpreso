...
function getInvoices() {
	try {
        ...
        $query = "SELECT i.invoice, c.name, i.invoice_date, i.invoice_due_date, i.invoice_type, i.status, c.email, i.custom_email, e.nama
                  FROM invoices i
                  JOIN customers c ON c.invoice = i.invoice
                  JOIN pegawai e ON e.id_pegawai = i.id_pegawai
                  ORDER BY i.invoice";
            ...
            while ($row = $results->fetch_assoc()) {
                echo '<tr>
                    ...
                    <td><a href="#" class="btn btn-primary btn-xs customer-select" 
                        ...
                        data-customer-postcode-ship="' . htmlspecialchars($row['postcode_ship']) . '">Pilih</a></td>
                </tr>';
            }
