...
$(document).ready(function () {
  ...
  // Tambahkan event listener untuk customer_county
  ...
  function actionCreateCustomer() {
    ...
    if (errorCounter > 0) {
      $("#response")
        ...
        .fadeIn();
      $("#response .message").html(
        "<strong>Error</strong>: It appears you have forgotten to complete something!"
      );
      $("html, body").animate({ scrollTop: $("#response").offset().top }, 1000);
    } else {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data && data.status && data.message) {
            ...
            $("#response .message").html(
              "<strong>Unexpected Success Response:</strong> Check console log."
            );
          }
        ...
        error: function (xhr, status, error) {
          console.log("Error Response:", xhr.responseText); // LOG ERROR
          $("#response .message").html(
            "<strong>Error</strong>: " +
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#create_invoice")
            .before()
            .html(
              "<a href='../invoice-create.php' class='btn btn-primary'>Create new invoice</a>"
            );
